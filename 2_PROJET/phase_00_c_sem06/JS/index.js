...
function gereRetour(json)
{
	...
	for(var i in action)
	{
		...
		case 'modifCfg':
			var e = getElem("resCfgMsg");
			//alert(action[i]);
			e.className = Object.keys(jsonObject["modifCfg"]) == "OK" ? "OK" : "KO";
			e.innerHTML = Object.keys(jsonObject["modifCfg"]) == "OK" ? jsonObject["modifCfg"]["OK"] : jsonObject["modifCfg"]["KO"];
			break;

		case 'fond':
			document.querySelector("body").style.backgroundColor = jsonObject['fond'];
			break;

		case 'changeLogo':
			console.log(retour);
			refElem("logo").src = retour.changeLogo + '?' + (new Date()).getTime();
			//getElem("logo").src = jsonObject.changeLogo;
			break;
		}
	}
