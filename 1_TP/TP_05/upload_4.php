        ...
        if(in_array($extentionUpload, $extentinsValides))
        {
            ...
            if(move_uploaded_file($_FILES['fichierUpload']['tmp_name'], $dossier . $fichier))
            {
	            ...
	            if($size[0] > $size[1])
	            {
		            ...
		            // placer dand la ressource que nous venons de créer une copie de l'image originelle,
	            ...
	            elseif ($extentionUpload == "png")
	            {
		            imagepng($im,$dossier.$fichier);
	            }
	            else
	            {
		           imagegif($im,$dossier.$fichier);
	            }
