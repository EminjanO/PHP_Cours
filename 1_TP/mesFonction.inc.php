...
{
    // P27(slide 3) P41
    $style = ["display:"."$index ===true .?block:none", 'font-style: italic'];
    $tbOut = "<table>";
    if(! empty($titre))
        $tbOut .= "<tr><th style='".implode(';',$style)."'>index</th><th>auteur</th><th>titrre</th><th>prix</th></tr>";
    if(is_array($liste))
    {
        reset($liste); // au cas où
        foreach($liste as $key => $val)
        {
            $tbOut .= "<tr><tb style='".implode(';',$style)."'>".$key."</tb><tb>".$val['auteur']."</tb><tb>".$val['titre']."</tb><tb>".$val['prix']."</tb>";
        }
...
creeTableau($tableau, 'avec titre', false);
/*
scriptInfos("name");
echo "<hr>";
scriptInfos("dns");
